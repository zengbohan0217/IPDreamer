...
class Trainer(object):
    def __init__(self,
        ...
        if 'ip-adapter' in self.guidance:
            self.guidance['ip-adapter'].offset_model.offset.requires_grad = True
            self.optimizer_offset = optim.Adam(itertools.chain(self.guidance['ip-adapter'].offset_model.parameters()), lr=0.0001, weight_decay=5e-4) # naive adam
            self.lr_scheduler_offset = optim.lr_scheduler.LambdaLR(self.optimizer_offset, lr_lambda=lambda epoch: 1) # fake scheduler
            self.scaler_offset = torch.cuda.amp.GradScaler()
        ...
        self.early_termi = False

        # Add multi-view supervision
        self.ref_gt_multi = None           # [6, 3, H, W]
    ...
    def train_step(self, data, save_guidance_path:Path=None):
        ...
        # novel view loss
            ...
            if 'SD' in self.guidance:
                ...
                if ('ip-adapter' in self.guidance) and ('normal_image' in outputs):
                    loss = loss + self.guidance['SD'].train_step(text_z, pred_normal, as_latent=as_latent, guidance_scale=self.opt.guidance_scale, grad_scale=self.opt.lambda_guidance,
            ...
            if 'ip-adapter' in self.guidance:
                ...
                # WARNING: consider how to combine the angle with the image prompt
                azimuth = data['azimuth'] # [-180, 180]
                if self.epoch > 50 and self.opt.dmtet:
                    self.guidance['ip-adapter'].max_step = int(self.guidance['ip-adapter'].num_train_timesteps * self.opt.ip_adapter_final_MaxT)

                if self.epoch > self.opt.ip_adapter_sds_epoch:
                    reference_img_path = self.opt.ip_adapter_ref_img           # the front sight
                    if self.opt.ip_adapter_geometry:
                        ip_ada_input = pred_normal
                    else:
                        ip_ada_input = pred_rgb

                    if os.path.isfile(reference_img_path):
                        refer_img = Image.open(reference_img_path).resize((512, 512))
                        if self.opt.ip_adapter_prompt_delta:
                            default_view_res, default_view_normal = self.obtain_default_render_res()
                            
                            # pred_default_prompt, _ = self.guidance['ip-adapter'].ip_model.get_image_embeds(default_view_res.clamp(0.0, 1.0).detach())
                            # pred_rgb_prompt, _ = self.guidance['ip-adapter'].ip_model.get_image_embeds(pred_rgb.clamp(0.0, 1.0).detach())
                            normal_default_prompt, _ = self.guidance['ip-adapter'].ip_model.get_image_embeds(default_view_normal.detach())
                            normal_rgb_prompt, _ = self.guidance['ip-adapter'].ip_model.get_image_embeds(pred_normal.detach())
                            
                            # img_prompt_delta = 0.4*(pred_rgb_prompt - pred_default_prompt) + 0.4*(normal_rgb_prompt - normal_default_prompt)
                            img_prompt_delta = 0.8*(normal_rgb_prompt - normal_default_prompt)
                            img_prompt_embeddings = self.guidance['ip-adapter'].get_prompt(refer_img, 1, self.opt.text, image_prompt_delta=img_prompt_delta)
                        else:
                            img_prompt_embeddings = None
                        
                        if azimuth[0] >= (-1*self.opt.ip_adapter_ang_scal) and azimuth[0] < self.opt.ip_adapter_ang_scal:                    # for bs=1
                            loss = loss + self.opt.ip_adapter_loss_w*self.guidance['ip-adapter'].train_step(refer_img, ip_ada_input, as_latent=as_latent,
                                                                                                            guidance_scale=self.opt.ip_adapter_cfg,img_prompt_embeddings=img_prompt_embeddings)
                        else:
                            loss = loss + self.opt.ip_adapter_loss_w

                        # if self.opt.dmtet:
                        #     lambda_guidance = 10 * (1 - abs(azimuth) / 180) * self.opt.lambda_guidance
                        #     loss = loss + lambda_guidance * self.guidance['ip-adapter'].IP_align_loss(refer_img, pred_rgb)
                    elif os.path.isdir(reference_img_path):
                        loss = loss + self.opt.ip_adapter_loss_w
    ...
    def obtain_tempo_input(self, thetas, phis, radius, H, W):
        # circle pose
        thetas = torch.FloatTensor([thetas]).to(self.device)
        phis = torch.FloatTensor([phis]).to(self.device)
        radius = torch.FloatTensor([radius]).to(self.device)
    ...
    def obtain_default_render_res(self):
        # for stage 1, dreamfusion
        H, W = self.opt.h, self.opt.w
        rays, mvp = self.obtain_tempo_input(thetas=self.opt.default_polar, phis=self.opt.default_azimuth, radius=self.opt.default_radius, H=H, W=W)
    ...
    def eval_step(self, data):
        ...
        bg_color = None
        if self.opt.ip_textureless == True:
            shading = 'textureless'
            ambient_ratio = 0
            bg_color = 1
        ...
        pred_normal = None
        if 'normal_image' in outputs:
            pred_normal = outputs['normal_image'].reshape(B, H, W, 3)#.permute(0, 3, 1, 2).contiguous()
    ...
    def train(self, train_loader, valid_loader, test_loader, max_epochs):
        ...
        for epoch in range(self.epoch + 1, max_epochs + 1):
            ...
            if self.epoch % self.opt.test_interval == 0 or self.epoch == max_epochs or self.early_termi:
    ...
    def train_one_epoch(self, loader, max_epochs):
        ...
        if self.opt.dmtet:
            ...
            elif self.best_loss < average_loss and (average_loss - self.best_loss) > 0.4:
                self.early_termi = True
    ...
    def evaluate_one_epoch(self, loader, name=None):
        ...
        with torch.no_grad():
            ...
            for data in loader:
                ...
                with torch.cuda.amp.autocast(enabled=self.fp16):
                    preds, preds_depth, preds_noraml, loss = self.eval_step(data)
                ...
                if self.local_rank == 0:
                    ...
                    if preds_noraml != None:
                        pred_noraml = preds_noraml[0].detach().cpu().numpy()
                        pred_noraml = (pred_noraml * 255).astype(np.uint8)
                        cv2.imwrite(save_path_normal, cv2.cvtColor(pred_noraml, cv2.COLOR_RGB2BGR))
